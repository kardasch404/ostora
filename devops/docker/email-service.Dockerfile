...
COPY --from=builder --chown=ostora:ostora /app/dist/apps/email-service ./dist
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=40s --retries=3 \
  CMD node -e "require('http').get('http://localhost:4721/health', (r) => {process.exit(r.statusCode === 200 ? 0 : 1)})"
