...
COPY apps/b2b-service ./apps/b2b-service
COPY libs ./libs
COPY prisma ./prisma

RUN npx prisma generate
RUN npx nx build b2b-service --prod
...
COPY --from=builder --chown=ostora:ostora /app/dist/apps/b2b-service ./dist
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=40s --retries=3 \
  CMD node -e "require('http').get('http://localhost:4726/health', (r) => {process.exit(r.statusCode === 200 ? 0 : 1)})"
